 -- 查询订单进度成功 V5.3  创建存储过程_实现数据刷新并储存（增加承诺交期字段）
 
 
 
 CREATE DEFINER=`root`@`localhost` PROCEDURE `refresh_order_progress`()
BEGIN
    -- 清空旧数据
    ...
    cte AS (
      SELECT
        ...
        po.deliver_time AS deliver_time,  -- 新增承诺交期字段
